...
namespace ERP_Transporte.Entidades
{
    internal class Estudante
    {
        ...
        {
            string sql = "SELECT e.id, e.nome AS Nome, CONCAT(e.logradouro, ', ', e.numero, ' - ', e.bairro) AS Endereço, " +
                "k.nome AS Escola, serie as Serie, r.nome AS Trajeto, CASE e.periodo WHEN 1 THEN 'M' WHEN 2 THEN 'T' ELSE 'I' END AS Período, responsavel as Responsavel, resp_qualif as 'Qualif. Resp.', " +
                "e.updated_at AS 'Ultima atualização' " +
                "FROM estudante e JOIN escola k ON k.id = e.id_escola JOIN rota r ON r.id = e.id_rota";
